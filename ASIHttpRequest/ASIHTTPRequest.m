...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		NSString *userAgentString = [[ASIHTTPRequest defaultUserAgentString] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
		if (userAgentString) {
	...
	if ([self partialDownloadSize]) {
      ...
      NSLog(@"userAgentString=%@",self.requestHeaders);
}
...
+ (NSString *)defaultUserAgentString
{
	...
	return [NSString stringWithFormat:@"kangyq%@ %@ (%@; %@ %@; %@)", appName, appVersion, deviceName, OSName, OSVersion, locale];
}
